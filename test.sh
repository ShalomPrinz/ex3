...
gcc -Wall -Wextra -Werror -std=c99 ex3.c -o program -lm -DNDEBUG
...
run_test_case() {
    ...
    if diff -Z actual_temp.txt expected_temp.txt > /dev/null; then
        ...
        echo "--- Differences ---"
        diff -Z -U 2 actual_temp.txt expected_temp.txt
        rm actual_temp.txt expected_temp.txt
        return 1
    fi
}
